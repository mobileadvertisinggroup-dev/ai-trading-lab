...
Version: FINAL-1.2.1 (FINAL-1.2 plus clarification PC-1 — source acquisition vs. project ingestion — explicitly approved by the user on 2026-08-25 after independent review)
Status: APPROVED FOR IMPLEMENTATION
Supersedes: FINAL-1.2, FINAL-1.1, and all prior drafts, amendments, and review correspondence. FINAL-1.2 and FINAL-1.1 are preserved unchanged as SPEC_FINAL-1.2.md / SPEC_FINAL-1.1.md for audit history. No other document is binding.
...
AKRA AI TRADING LAB must remain completely independent from all previous projects, including for verification. No previous project — including Engine v7 and binance-execution-manager — may be attached, inspected, copied, imported, called, or used.
...
3. Phase 2: Data ingestion and holdout quarantine—the holdout range is mechanically established by the frozen Phase-1 partition rule applied during the sealing process, before any project ingestion (PC-1)
4. Phase 3: Independent simulator
5. Phase 4: Arm A implementation
6. Phase 5: Golden fixtures and Independent Reference Ledger differential gate
7. Phase 6: Arms B–G
...
3a. (PC-1) Source acquisition vs. project ingestion. A non-interactive sealing process MAY acquire the complete public source history into isolated, ephemeral runner storage solely to: (i) validate data quality mechanically; (ii) determine the eligible continuous interval; (iii) compute the binding 60/20/20 boundary; (iv) divide the source stream into pre-holdout and holdout records; (v) encrypt the holdout records. "Project ingestion" means admission into the ordinary readable project data lake; no holdout-period row may enter that lake. Acquisition staging MUST be: inaccessible to model training, validation, dashboard, and ordinary diagnostic code; free of any display of holdout values, rows, summaries, or outcomes; limited to metadata-only logs; never committed to Git; never uploaded as an ordinary Actions artifact; never cached between runs; destroyed immediately after successful sealing or on failure, with the run FAILING if destruction cannot be verified. Only pre-holdout data enters the readable lake; holdout data goes directly from isolated acquisition staging into the encrypted seal.
...
* Import of any previous project
* Modification of any previous project
* Runtime dependency on any previous project (Engine v7 included)
...
13. INDEPENDENT REFERENCE LEDGER DIFFERENTIAL GATE

(Replaces FINAL-1.1's mandatory Engine v7 differential gate, by explicit user authorization on 2026-08-25, before Phase 5, model training, constitutional-test locking, shakedown, or official evaluation.)

Build a new, deliberately minimal reference ledger specifically for AKRA AI TRADING LAB.

It must:

* Be implemented independently from the main simulator.
* Live in a clearly separate module or package.
* Share no accounting, execution, portfolio, fee, stop, sizing, or P&L implementation code with the main simulator.
* Never import the main simulator.
* Never be imported by the production simulator.
* Use the same frozen semantic specification as an input, but implement calculations independently.
* Be used only for verification.
* Remain simple enough for manual inspection.
* Produce a transaction-level ledger suitable for differential comparison.

The minimum required differential subset remains:
...
Use independently reviewed golden fixtures that exercise every required behavior.

Verification must include three independent layers:

1. Manually derived expected fixture results reviewed by the user, ChatGPT, or another independent reviewer
2. Independent Reference Ledger results
3. Main simulator results

The main simulator and Independent Reference Ledger must reconcile exactly on the formally shared semantic subset.

On mismatch:

1. Stop the verification gate.
2. Identify the first transaction-level divergence.
3. Produce a differential report.
4. Determine whether the error is in:
    * The main simulator
    * The reference ledger
    * The fixture
    * The frozen semantic specification
5. Never assume either implementation is automatically correct.
6. Correct the defective component through the approved test-governance process.
7. Rerun the complete gate.

Add property-based accounting tests covering invariants such as:

* Cash and equity conservation
* Fees never increasing equity
* Closing a flat position producing no trade P&L
* Long and short symmetry under mirrored prices and identical costs
* Exposure never exceeding the permitted cap
* Rejected orders never changing cash, equity, or positions
* Partial exits never closing more than the open quantity
* Position quantity never becoming negative
* Insolvent accounts never opening new risk
* Identical deterministic inputs producing identical ledgers

The golden-fixture and independent-review requirements of Section 12 are not weakened by this replacement.
...
* Reference-ledger reconciliation
...
* Risk policy
* Simulator semantics
* Reference-ledger differential specification
...
8. Rerun the Independent Reference Ledger reconciliation when shared semantics changed.
...
* Independent Reference Ledger reconciliation results
...
* Independent Reference Ledger minimum-subset reconciliation passes.
...
APPENDIX A2 — FINAL-1.2 CHANGE LOG (2026-08-25, user-authorized)

1. The mandatory Engine v7 differential gate (former Section 13) was removed and replaced everywhere by the Independent Reference Ledger differential gate.
2. Total-independence mandate added: no previous project — including Engine v7 and binance-execution-manager — may be attached, inspected, copied, imported, called, or used, including for verification.
3. Property-based accounting invariant tests were added to the verification gate.
4. The minimum differential subset, golden-fixture requirements, independent-review requirements, and mismatch-adjudication procedure were retained unweakened.
5. Authorization: explicit user instruction, 2026-08-25. Reason: preserve total independence from previous projects while retaining differential verification. Timing: before data-derived model training, constitutional-test locking, shakedown, holdout evaluation, or official results. Affected phase: Phase 5 verification only. No existing results or models required invalidation. Blocker B2 (Engine v7 availability) permanently closed as REMOVED BY APPROVED SPECIFICATION CHANGE. No permission exists to attach binance-execution-manager.

⸻

APPENDIX A3 — FINAL-1.2.1 CHANGE LOG (2026-08-25, user-approved after independent review)

1. Clarification PC-1 adopted: §9.3a distinguishes source acquisition (ephemeral, protected, verifiably destroyed staging inside the sealing process) from project ingestion (admission into the readable lake); §2 phase-list item 3 reworded accordingly.
2. R53 binding reading: the protocol foundation (the RULES — Arm A, universe, data-quality, partition) is frozen before ingestion; the concrete DATES those rules produce are computed mechanically inside the sealing process per §9.3a.
3. No other section changed; the experiment's information structure is unchanged (the readable lake still contains zero holdout-period rows; the §8 builder-knowledge caveat is unchanged).
4. Approval trail: proposed as PROPOSED_CLARIFICATION_PC1.md per the Phase-6 review verdict §4; independently reviewed; explicitly approved by the user 2026-08-25 ("PC-1 is approved by us").
...
R26	Independent simulator; no dependency on any previous project	11
R27	Golden fixtures require independent review	12
R28	Independent Reference Ledger gate has meaningful minimum subset	13
...
R56	Reference ledger fully independent: separate module, no shared accounting code, no imports in either direction	13
R57	Property-based accounting invariant tests required	13
R58	No previous project attached, inspected, copied, imported, called, or used	1
